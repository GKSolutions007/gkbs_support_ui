class GKBSDynamicGrid {
        ...
        if (this.state.searchTerm) {
            ...
            result = result.filter(row => {
                return Object.values(row).some(val => {
                    ...
                    if (typeof val === 'symbol') return false;  // ✅ Skip Symbols
    ...
    updateCalculatedFields(updatedRow) {
        ...
        try {
            const gridIdStr = updatedRow._gridId ? updatedRow._gridId.toString() : '';
            if (gridIdStr) {
                const rowEl = this.container.querySelector(`.dg-row[data-grid-id="${gridIdStr}"]`);
                if (rowEl) {
                    const netSalaryCellContainer = rowEl.querySelector(`.dg-cell[data-field="netsalary"]`);
                    if (netSalaryCellContainer && netSalaryCellContainer.firstChild) {
                        netSalaryCellContainer.firstChild.innerText = netSalary.toFixed(2);
                    }
                }
            }
        } catch (e) {
            // Symbol conversion handled safely
        }
    ...
    renderStatusBarAfterChange() {
        ...
        // ✅ Pass scrollWrapper as required argument
        this.renderStatusBar(scrollWrapper);
